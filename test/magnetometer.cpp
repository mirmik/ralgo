...
TEST_CASE("magnetometer") 
{
	...
	auto collector = ralgo::spherical_cloud_collector(yaw_total, pitch_total, array);
	...
	CHECK_EQ(collector.pitch_by_index(1), doctest::Approx(-M_PI / 2 + M_PI/5));
	...
	CHECK_EQ(collector.index_by_angles({0.7,M_PI/2-0.001}), yaw_total*(pitch_total-2)+1);
	...
	CHECK_EQ(collector.index_by_angles({2*M_PI-0.01,-M_PI / 2 + M_PI/5}), 6);
