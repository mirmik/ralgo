...
namespace ralgo 
{
	...
	public:
        ...
        double mark_velocity_control(
        	double current_pos, double target_vel, double current_vel, 
                float alpha, double delta) 
        {
        	apply_mark_speed(target_vel, delta);
        	nos::println("mark_position", mark_position);
        	return position_velocity_control(
        		mark_position, current_pos, mark_velocity, current_vel, 
                	alpha, delta);
        } 

        double mark_position_control(
        	double target, 
        	double current_pos, 
        	double current_vel, 
            float alpha, double delta) 
        {
        	...
        	return mark_velocity_control(current_pos, target_vel, current_vel, 
                alpha, delta);
        }
	};
}
