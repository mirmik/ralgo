...
namespace ralgo 
{
    ...
    public:
        ...
        double torque_control(
        	double target_torq, double current_torq, double delta)
        {
        	return torq_pi.serve(target_torq - current_torq, delta);
        }

        double velocity_control(
        	double target_vel, double current_vel, double delta)
        {
            return vel_pi.serve(target_vel - current_vel, delta);
        }

        double position_control(
        	double target_pos, double current_pos, double delta)
        {
        ...
        double position_velocity_control(
        	double target_pos, double current_pos, double target_vel, double current_vel, 
                float alpha, double delta) 
        {
        	auto target_vel_by_pos  = position_control(target_pos, current_pos, delta);
            auto mean_target_vel = target_vel_by_pos * (alpha) + target_vel * (1-alpha);
