...
{
    template <class T>
    ralgo::vector<T> roots(T b, T a)
    {
        return { -b/a };
    } 

    template <class T>
    ralgo::vector<T> roots(T c, T b, T a)
    {
        double d = b*b - 4*a*c;
        if (d < 0) 
        {
    ...
    static inline ralgo::vector<double> roots(ralgo::vector<double> coeffs)
    {
        ...
        if (coeffs.size() == 2) { return roots(coeffs[0], coeffs[1]); }
        if (coeffs.size() == 3) { return roots(coeffs[0], coeffs[1], coeffs[2]); }
