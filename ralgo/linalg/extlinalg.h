...
namespace ralgo 
{
    ...
    template <class T>
    linalg::mat<T,3,3> diag(const linalg::vec<T,3>& v) 
    {
        return {
            {v[0], 0, 0},
            {0, v[1], 0},
    ...
    template <class T>
    linalg::mat<T,3,3> outer(const linalg::vec<T,3>& v1, const linalg::vec<T,3>& v2) 
    {
        return {
    ...
    template <class T>
    linalg::mat<T, 3, 3> rotate_quadric_form(
