...
namespace ralgo {
    ...
    void U_triangle_inverse(const A &a, R &inv)
    {
    ...
    defsame_t<R, ralgo::matrix<typename M::value_type>> U_triangle_inverse(const M& a)
    {
    ...
    template <class R, class A>
    void L_triangle_inverse(const A &a, R &inv)
    {
        inv.resize(a.rows(), a.cols());
        ralgo::matops::eye(inv);
    ...
    template <class R = void, class M>
    defsame_t<R, ralgo::matrix<typename M::value_type>> L_triangle_inverse(const M& a)
    {
        defsame_t<R, ralgo::matrix<typename M::value_type>> inv;
