...
// src - массив анализируемых данных, srclen - длина массива должна быть кратна степени 2.
// dst - массив полученных значений, dstlen - длина массива должна быть равна srclen.

const double TwoPi = 6.283185307179586;

void ralgo::signal::fft_reference (std::vector<std::complex<double>> & a, bool invert) 
{
	int n = (int) a.size();
	if (n == 1)  return;
...
void ralgo::signal::spectre(double *src, double *dst, int srclen, int dstlen)
{
	...
	for (i = 0; i < dstlen; i++)
	{
		j = i * 2; dst[i] = 2 * sqrt(pow(Tmvl[j], 2) + pow(Tmvl[j + 1], 2)) / srclen;
	}

	delete []Tmvl;
}

std::vector<double> ralgo::signal::spectre(double *src, size_t srclen, size_t dstlen) 
{
