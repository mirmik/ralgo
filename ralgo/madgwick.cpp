...
// AHRS algorithm update
        ...
        float mx, float my, float mz)
    {
        ...
        hx = 2.0f * (mx * (0.5f - q2q2 - q3q3) + my * (q1q2 - q0q3) + mz * (q1q3 + q0q2)); // проверено
        hy = 2.0f * (mx * (q1q2 + q0q3) + my * (0.5f - q1q1 - q3q3) + mz * (q2q3 - q0q1)); // проверено
        //bx = sqrt(hx * hx + hy * hy);
...
// IMU algorithm update
    ...
    void madgwick::apply(float gx, float gy, float gz, float ax, float ay, float az)
    {
        float module;
        float recipNorm;
        float s0, s1, s2, s3;
...
// See: http://en.wikipedia.org/wiki/Fast_inverse_square_root
    ...
    {
        //if (x==0) return 0;
        return 1 / sqrt(x);  //quick_invsqrt(x);
    }
    ...
    {
        return -1 * atan2(2.0f * (q0 * q2 - q1 * q3), 1.0f - 2.0f * (q2 * q2 + q1 * q1 ));
    }
