...
namespace heimer
{
    ...
    public:
        ...
        StatusPair bind_signals(igris::array_view<wishfeed *> left,
                                igris::array_view<wishfeed *> right)
        {
            ...
            for (unsigned int i = 0; i < left.size(); ++i)
            {
                if (left[i]->right != nullptr)
                    return {Status::SIGNAL_CONNECTION_CONFLICT,
                            left[i]->name()};
        ...
        */
        ...
        ralgo::matrix_view_co<real>
        signals_as_matrix(igris::array_view<wishfeed *> inbuf,
                          getter_ptr getter, real *signal_buffer)
        {
        ...
        // В зависимости от getter может работать с wish ли feed частью
        void set_signals(const ralgo::matrix_view_co<real> &result,
                         getter_ptr getter,
        ...
        void serve_feed()
        {
            process(left_signals(), right_signals(), &wishfeed::feed,
                    &wishfeed_node::do_feed_transform);
        }

        void serve_wish()
        {
            process(right_signals(), left_signals(), &wishfeed::wish,
                    &wishfeed_node::do_wish_transform);
        }

        void process(igris::array_view<wishfeed *> inbuf,
                     igris::array_view<wishfeed *> outbuf, getter_ptr getter,
        ...
        virtual void do_feed_transform(real *signal, real *result, int sigcount,
                                       int sigdim) = 0;
        virtual void do_wish_transform(real *signal, real *result, int sigcount,
                                       int sigdim) = 0;
    };
}
