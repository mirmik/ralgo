...
namespace heimer
{
    ...
    public:
        bool is_used() { return _refs != 0; }

        static int typehint_cast(const char *strhint)
        {
            if (strcmp(strhint, "servowf") == 0)
                return DATANODE_TYPEHINT_SERVOWISHFEED;
        ...
        datanode(const char *name, const char *hint) { init(name, hint); }

        datanode(const char *name, int hint) { init(name, hint); }
        ...
        int typehint() const { return _typehint; }
