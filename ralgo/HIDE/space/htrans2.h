...
namespace ralgo
{
    ...
    template <typename T> struct htrans2
    {
        T orient;
        ...
        {
            return htrans2(orient + oth.orient,
                           linalg::rot(orient, oth.center) + center);
        }

        screw2<T> operator-(const htrans2<T> &oth)
        {
            return {orient - oth.orient, center - oth.center};
        }
        ...
        htrans2 inverse() { return {-orient, linalg::rot(-orient, -center)}; }
